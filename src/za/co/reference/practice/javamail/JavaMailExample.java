package za.co.reference.practice.javamail;
...
public class JavaMailExample{
	...
	private void forwardMessage() throws AddressException, MessagingException{
		/*
		 * Forwarding messages is a little more involved. There is no single method to call, and you build up 
		 * 	the message to forward by working with the parts that make up a message. 

			A mail message can be made up of multiple parts. Each part is a BodyPart, or more specifically, a 
			MimeBodyPart when working with MIME messages. The different body parts get combined into a 
			container called Multipart or, again, more specifically a MimeMultipart. To forward a message, you 
			create one part for the text of your message and a second part with the message to forward, and 
			combine the two into a multipart. Then you add the multipart to a properly addressed message and 
			send it.
